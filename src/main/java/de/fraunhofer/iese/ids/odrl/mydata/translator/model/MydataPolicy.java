...
public class MydataPolicy {
	...
	private String getMechanismsBlock() {
		StringBuilder mechanismsBlockBuilder = new StringBuilder();
		if (null != this.mechanisms && !this.mechanisms.isEmpty()) {
			for (MydataMechanism mechanism : this.mechanisms) {
				mechanismsBlockBuilder.append(mechanism.toString());
			}
		}
		return mechanismsBlockBuilder.toString();
	}
