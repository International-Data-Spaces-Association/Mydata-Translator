			...
			for (Rule rule : odrlPolicy.getRules()) {
				...
				if (null != action) {
					...
					if (null != rule.getTarget()) {
						this.addTargetConstraintToMechansim(mydataMechanism, rule.getTarget().toString());
					}
					...
					if (null != rule.getConstraints()) {
						for (Condition odrlConstraint : rule.getConstraints()) {
							...
							case PURPOSE:
								this.addPurposeConstraintToMechanism(mydataMechanism, odrlConstraint);								break;
							case EVENT:
								mydataMechanism = this.eventConstraint(mydataMechanism, odrlConstraint);
								break;
							case ABSOLUTE_SPATIAL_POSITION:
								this.absoluteSpatialPositionConstraintToMechanism(mydataMechanism,
										odrlConstraint);
								break;
							case PAY_AMOUNT:
								...
								break;
							case SYSTEM:
								addSystemConstraintToMechanism(mydataMechanism, odrlConstraint);
								break;
							case APPLICATION:
							...
							case DATE_TIME:
								addDateTimeConstraintToMechanism(mydataMechanism, odrlConstraint);
								break;
							case COUNT:
...
//								break;
					...
					if (null != rule.getPreduties() && !rule.getPreduties().isEmpty()) {
						for (Rule preobligation : rule.getPreduties()) {
							...
							if (actionType.equals(ActionType.ANONYMIZE) || actionType.equals(ActionType.REPLACE)
	...
	private MydataMechanism elapsedTimeConstraint(MydataMechanism mydataMechanism, Condition odrlConstraint) {
		if (null != odrlConstraint) {
			...
			// semantically, the elapsed time conditions cannot get more than one right
			// operand; time conflicts occurs.
			for (RightOperandEntity entity : odrlConstraint.getRightOperands().get(0).getRightOperandEntities()) {
				switch (entity.getEntityType()) {
				case BEGIN:
					RightOperandEntity beginInnerEntity = entity.getInnerEntity();
	...
	private MydataMechanism encodingConstraint(MydataMechanism mydataMechanism, Condition odrlConstraint) {
		if (null != odrlConstraint) {
			List<Parameter> pipParams = new ArrayList<>();
			for (RightOperand rightOperand : odrlConstraint.getRightOperands()) {
				Parameter encodingParam = new Parameter(ParameterType.STRING,
...
//			} else 
	...
	private void absoluteSpatialPositionConstraintToMechanism(MydataMechanism mydataMechanism,
			Condition odrlConstraint) {
		if (null != odrlConstraint) {
			List<Parameter> pipParams = new ArrayList<>();
			// list of locations
			for (RightOperand rightOperand : odrlConstraint.getRightOperands()) {
			...
			mydataMechanism.getPipBooleans().add(locationPipBoolean);
		}
	}


	private void addDateTimeConstraintToMechanism(MydataMechanism mydataMechanism, Condition odrlConstraint) {
		if (PatternUtil.isNotNull(odrlConstraint)) {
			// semantically, the time conditions cannot get more than one right operand;
			// time conflicts occurs. TODO: WHAT?

			List<RightOperand> rightOperands = odrlConstraint.getRightOperands();
			
			List<RightOperandEntity> rightOperandEntities = rightOperands.get(0)
					.getRightOperandEntities();

			if (PatternUtil.isNotEmpty(rightOperandEntities)) {
				for (RightOperandEntity rightOperandEntity : rightOperandEntities) {
					...
					case BEGIN:
						...
						mydataMechanism.getDateTimes().add(startTime);
						break;
					case END:
	...
	private void addPurposeConstraintToMechanism(MydataMechanism mydataMechanism, Condition odrlConstraint) {
		if (null != odrlConstraint) {
			List<Parameter> pipParams = new ArrayList<>();
			// list of purposes
			for (RightOperand rightOperand : odrlConstraint.getRightOperands()) {
			...
			mydataMechanism.getPipBooleans().add(purposePipBoolean);
		}
	}

	private void addSystemConstraintToMechanism(MydataMechanism mydataMechanism, Condition odrlConstraint) {
		if (null != odrlConstraint) {
			List<Parameter> pipParams = new ArrayList<>();
			// list of systems
			for (RightOperand rightOperand : odrlConstraint.getRightOperands()) {
			...
			mydataMechanism.getPipBooleans().add(systemPipBoolean);
		}
	}
